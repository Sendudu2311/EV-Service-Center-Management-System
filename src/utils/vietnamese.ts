...
export const isValidVietnamesePhone = (phone: string): boolean => {
  ...
  const landlinePattern =
    /^(02[4-9]|024|028|023|025|026|027|029|022)[0-9]{7,8}$/;
  ...
  try {
    ...
    if (dateStr.includes("T") || dateStr.includes("Z")) {
      ...
      if (parts.length === 3) {
        ...
        if (third.length === 4) {
          isoDateStr = `${third}-${second.padStart(2, "0")}-${first.padStart(
            2,
            "0"
          )}`;
        } else {
          // Assume MM/DD/YYYY
          isoDateStr = `${third}-${first.padStart(2, "0")}-${second.padStart(
            2,
            "0"
          )}`;
        }
...
export const vietnameseValidationPatterns = {
  phone:
    /^(09|08|07|05|03)[0-9]{8}$|^(02[4-9]|024|028|023|025|026|027|029|022)[0-9]{7,8}$/,
  email: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
  vietnameseName:
