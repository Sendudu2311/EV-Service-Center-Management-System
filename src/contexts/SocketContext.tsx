  ...
  useEffect(() => {
    if (isAuthenticated && token && user) {
      ...
      socketInstance.on('connect', () => {
        ...
        if (!isConnected) {
          toast.success('Kết nối cập nhật thời gian thực thành công', { duration: 3000 });
        }
      });
      ...
      socketInstance.on('connect_error', (error) => {
        ...
        if (document.visibilityState === 'visible') {
          toast.error('Mất kết nối cập nhật thời gian thực', { duration: 4000 });
        }
      });
      ...
      socketInstance.on('appointment_status_updated', (data) => {
        ...
        const isUserInvolved = data.customerId === user._id || data.technicianId === user._id;
        if (isUserInvolved) {
          toast.info(`Lịch hẹn ${data.appointmentNumber}: ${data.status}`, { duration: 4000 });
        }

        window.dispatchEvent(new CustomEvent('appointmentStatusUpdate', { detail: data }));
      });
      ...
      socketInstance.on('new_appointment', (data) => {
        ...
        if (user.role === 'staff' || user.role === 'admin') {
          toast.info(`Lịch hẹn mới: ${data.appointmentNumber}`, { duration: 5000 });
        }
      ...
      socketInstance.on('technician_assigned', (data) => {
        ...
        if (user._id === data.technicianId) {
          toast.success(`Bạn được phân công lịch hẹn: ${data.appointmentNumber}`, { duration: 6000 });
        }
        window.dispatchEvent(new CustomEvent('technicianAssigned', { detail: data }));
      });
      ...
      socketInstance.on('parts_requested', (data) => {
        ...
        if ((user.role === 'staff' || user.role === 'admin') && hour >= 8 && hour <= 18) {
          toast.info(`Yêu cầu phụ tùng: ${data.appointmentNumber}`, { duration: 4000 });
        }
        window.dispatchEvent(new CustomEvent('partsRequested', { detail: data }));
      });
      ...
      socketInstance.on('parts_approved', (data) => {
        ...
        if (user._id === data.requestedBy) {
          toast.success(`Phụ tùng đã được duyệt: ${data.requestNumber}`, { duration: 5000 });
        }
        window.dispatchEvent(new CustomEvent('partsApproved', { detail: data }));
      });
      ...
      socketInstance.on('invoice_generated', (data) => {
        ...
        if (user._id === data.customerId) {
          toast.success(`Hóa đơn mới: ${data.invoiceNumber}`, { duration: 5000 });
        }
        window.dispatchEvent(new CustomEvent('invoiceGenerated', { detail: data }));
      });

      socketInstance.on('payment_received', (data) => {
        console.log('Payment received:', data);
        // Only notify staff/admin, not every payment
        if ((user.role === 'staff' || user.role === 'admin') && data.amount > 1000000) {
          toast.success(`Thanh toán lớn: ${data.invoiceNumber}`, { duration: 4000 });
        }
      ...
      socketInstance.on('new_message', (data) => {
        ...
        if (data.senderId !== user._id && !window.location.pathname.includes('/chat')) {
      ...
      socketInstance.on('service_reception_created', (data) => {
        ...
        if (user.role === 'admin' || (user.role === 'staff' && user.isManager)) {
          toast.info(`Phiếu tiếp nhận: ${data.appointmentNumber}`, { duration: 4000 });
        }
        window.dispatchEvent(new CustomEvent('serviceReceptionCreated', { detail: data }));
      });

      socketInstance.on('service_reception_approved', (data) => {
        console.log('Service reception approved:', data);
        if (user._id === data.technicianId) {
          toast.success(`Phiếu tiếp nhận đã duyệt: ${data.appointmentNumber}`, { duration: 4000 });
        }
        window.dispatchEvent(new CustomEvent('serviceReceptionApproved', { detail: data }));
      });
...
export const useCustomEvent = (eventName: string, handler: (data: any) => void) => {
  useEffect(() => {
