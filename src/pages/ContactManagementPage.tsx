    ...
    <div className="min-h-screen bg-dark-900">
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-text-secondary">Manage customer inquiries and support messages</p>
        </div>
        {/* Filters */}
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <input
                ...
                className="w-full px-4 py-2 bg-dark-300 text-white border border-dark-200 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-lime-400 placeholder-text-muted"
              />
            </div>
            <div>
              <select
                ...
                className="px-4 py-2 bg-dark-300 text-white border border-dark-200 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-lime-400"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Contacts List */}
          <div className="bg-dark-300 rounded-lg shadow">
              ...
              {contacts.length === 0 ? (
                ...
                contacts.map((contact) => (
                  <div
                    ...
                    className={`p-4 cursor-pointer hover:bg-dark-900 transition-colors ${
                      selectedContact?._id === contact._id ? 'bg-dark-900 border-r-4 border-blue-500' : ''
                    } ${!contact.isRead ? 'bg-lime-25' : ''}`}
                  >
                    ...
                    <p className="text-sm text-text-secondary mb-1">{contact.email}</p>
                    <p className="text-sm text-gray-800 mb-2 line-clamp-2">{contact.message}</p>
                    <div className="flex justify-between items-center text-xs text-text-muted">
                      <span>{contact.timeAgo}</span>
                      <span className={`text-text-muted ${getPriorityColor(contact.priority)}`}>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="p-4 border-t border-dark-200 flex justify-between items-center">
                <button
                  onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                  disabled={currentPage === 1}
                  className="px-3 py-1 text-sm bg-dark-100 hover:bg-dark-200 disabled:opacity-50 disabled:cursor-not-allowed rounded"
                >
                  Previous
                </button>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-text-secondary">{selectedContact.email}</p>
                  </div>
                  <div className="flex gap-2">
                    <select
                      value={selectedContact.status}
                      onChange={(e) => handleStatusChange(selectedContact._id, e.target.value)}
                      className="px-3 py-1 text-sm border border-dark-300 rounded focus:ring-2 focus:ring-lime-400"
                    >
                      <option value="open">Open</option>
                      <option value="in-progress">In Progress</option>
                      <option value="closed">Closed</option>
                ...
                <div className="mb-6">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <span className={`text-sm text-text-muted ${getPriorityColor(selectedContact.priority)}`}>
                    ...
                    <div className="space-y-3">
                      {selectedContact.notes.map((note, index) => (
                        <div key={index} className="bg-dark-900 rounded-lg p-3">
                          <div className="flex justify-between items-start mb-2">
                            <span className="text-sm text-text-muted text-lime-900">
                          ...
                          <p className="text-lime-800">{note.content}</p>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-dark-300 rounded-lg p-6 w-full max-w-md">
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 border border-dark-300 rounded-lg focus:ring-2 focus:ring-lime-400 focus:border-transparent mb-4"
              />
              <div className="flex justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2 text-text-secondary hover:text-gray-800"
                >
                  Cancel
                </button>
                <button
