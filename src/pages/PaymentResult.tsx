...
const PaymentResult: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (vnp_ResponseCode && vnp_TxnRef) {
      ...
      if (isVnPaySuccess) {
        ...
        toast.success('Payment successful! Please complete your appointment booking.');
  ...
  const verifyPaymentAndBookAppointment = async (transactionRef: string) => {
    try {
      // Verify the payment
      const verifyResponse = await vnpayAPI.verifyAppointmentPayment({ transactionRef });
      ...
      setPaymentInfo({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to verify payment or book appointment'
      });
    }
  };
            ...
            <>
              ...
              <p className="text-gray-600 mb-4">
                ...
                {paymentInfo?.autoCreated && (
                  <span className="text-green-600 font-medium"> Please complete your appointment booking.</span>
                )}
              </p>
              {paymentInfo && (
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{formatCurrency(parseFloat(paymentInfo.amount))}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Transaction:</span>
                      <span className="font-medium text-sm">{paymentInfo.transactionRef}</span>
                    </div>
                    {paymentInfo.vnpayTransaction && (
                      <>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Bank:</span>
                          <span className="font-medium text-sm">{paymentInfo.vnpayTransaction.bankCode}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Card Type:</span>
                          <span className="font-medium text-sm">{paymentInfo.vnpayTransaction.cardType}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">VNPay Transaction:</span>
                          <span className="font-medium text-sm">{paymentInfo.vnpayTransaction.transactionNo}</span>
                        </div>
                      </>
                    )}
                    {paymentInfo.appointmentId && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-sm text-green-600">{paymentInfo.appointmentId}</span>
                      </div>
                    )}
                    {paymentInfo.autoCreated && (
              ...
              <p className="text-sm text-gray-500 mb-4">
                You will be redirected to complete your appointment booking in a few seconds...
              </p>
              <button
