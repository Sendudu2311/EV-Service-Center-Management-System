...
const ServiceHistoryPage: React.FC = () => {
  ...
  const [selectedAppointment, setSelectedAppointment] =
    useState<CompletedAppointment | null>(null);
  ...
  const fetchCompletedAppointments = async () => {
    try {
      ...
      const completedData = Array.isArray(completedResponse.data?.data)
        ? completedResponse.data.data
        : Array.isArray(completedResponse.data)
          ? completedResponse.data
          : [];
      const invoicedData = Array.isArray(invoicedResponse.data?.data)
        ? invoicedResponse.data.data
        : Array.isArray(invoicedResponse.data)
          ? invoicedResponse.data
          : [];
        ...
        {loading ? (
          ...
          <div className="bg-dark-300 rounded-lg shadow-sm border border-dark-200 p-12 text-center">
            ...
            <p className="text-text-secondary">
              Chưa có dịch vụ nào được hoàn thành hoặc thanh toán
            </p>
          </div>
        ) : (
                    ...
                    <div className="mb-4">
                      ...
                      <ul className="space-y-2">
                        {appointment.services.map((service, index) => {
                          ...
                          try {
                            warrantyStatus = service.serviceId.warranty
                              ? getWarrantyStatus(
                                  appointment.actualCompletion,
                            ...
                            console.error(
                              "Error calculating warranty status for service:",
                              error
                            );
                            warrantyStatus = null;
                          }
                          ...
                          return (
                            <li
                              key={index}
                              className="flex items-start justify-between gap-2 p-3 bg-dark-900 rounded-md"
                            >
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {service.serviceId.warranty &&
                                    warrantyStatus && (
                                      <span
                                        className={`inline-flex items-center gap-1 px-2 py-0.5 text-xs text-text-muted rounded-full ${getWarrantyBadgeColor(warrantyStatus)}`}
                                      >
                                ...
                                {service.serviceId.warranty && (
                                  <p className="text-xs text-text-secondary">
                                    Bảo hành:{" "}
                                    {formatWarrantyDuration(
                                      service.serviceId.warranty.duration
                                    )}{" "}
                                    - {service.serviceId.warranty.description}
                                    {warrantyStatus && (
                                      <span className="ml-2 text-lime-600">
                                        (Hết hạn:{" "}
                                        {warrantyStatus.expiryDate.toLocaleDateString(
                                          "vi-VN"
                                        )}
                                        )
                                      </span>
                        ...
                        <div className="mb-4">
                          ...
                          <ul className="space-y-2">
                            {appointment.partsUsed.map((part, index) => {
                              // Check if partId exists before accessing properties
                              if (!part || !part.partId) {
                                return null;
                              }
                              ...
                              let warrantyStatus = null;
                              try {
                                warrantyStatus = part.partId.warranty
                                  ? getWarrantyStatus(
                                      appointment.actualCompletion,
                                ...
                                console.error(
                                  "Error calculating warranty status for part:",
                                  error
                                );
                                warrantyStatus = null;
                              }
                              ...
                              return (
                                <li
                                  key={index}
                                  className="flex items-start justify-between gap-2 p-3 bg-dark-900 rounded-md border border-blue-200"
                                >
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                      <span className="text-sm text-text-muted text-white">
                                    ...
                                    <p className="text-xs text-text-secondary">
                                      Mã: {part.partId.partNumber} • Số lượng:{" "}
                                      {part.quantity}
                                    </p>
                                    {part.partId.warranty && (
                                      <p className="text-xs text-lime-700 mt-1">
                                        Bảo hành:{" "}
                                        {formatWarrantyDuration(
                                          part.partId.warranty.duration
                                        )}{" "}
                                        - {part.partId.warranty.description}
                                        {warrantyStatus && (
                          ...
                          <ul className="space-y-2">
                            {appointment.serviceReceptionId.externalParts.map(
                              (part, index) => (
                                <li
                                  key={index}
                                  className="flex items-start justify-between gap-2 p-3 bg-amber-50 rounded-md border-2 border-amber-400"
                                >
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <p className="text-xs text-amber-800">
                                      Mã: {part.partNumber || "EXTERNAL"} • Số
                                      lượng: {part.quantity}
                                    </p>
                                    {part.supplier && (
                    ...
                    {appointment.feedback && (
                      <div className="mb-4 p-3 bg-yellow-50 rounded-md border border-yellow-200">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm text-text-muted text-yellow-900">
                            Đánh giá của bạn:
                          </span>
                ...
                <div className="bg-dark-300 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                  ...
                  <div className="space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    {selectedAppointment.serviceNotes &&
                      selectedAppointment.serviceNotes.length > 0 && (
                        <div>
                          ...
                          <div className="space-y-2">
                            {selectedAppointment.serviceNotes.map(
                              (note, index) => (
                                <div
                                  key={index}
                                  className="p-3 bg-dark-900 rounded-md border border-dark-200"
                                >
                                  ...
                                  <p className="text-xs text-text-muted">
                                    ...
                                    {formatVietnameseDateTime(note.addedAt)}
                                  </p>
                                </div>
                              )
                            )}
                    ...
                    {/* Workflow History */}
                    {selectedAppointment.workflowHistory &&
                      selectedAppointment.workflowHistory.length > 0 && (
                        <div>
                          <h4 className="text-md text-text-muted text-white mb-3">
                            Lịch sử tiến trình
                          </h4>
                          <div className="space-y-2">
                            {selectedAppointment.workflowHistory.map(
                              (history, index) => (
