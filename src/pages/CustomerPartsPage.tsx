...
const CustomerPartsPage: React.FC = () => {
  ...
  const getCategoryBadgeColor = (category: string) => {
    const colors: Record<string, string> = {
      ...
      charger: 'bg-blue-100 text-blue-800',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-600">Đang tải danh sách phụ tùng...</p>
          </div>
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className={`px-2 py-1 text-xs font-medium rounded-full ${getCategoryBadgeColor(part.category)}`}>
                        {part.category}
                      </span>
                    </div>
                    ...
                    <div className="mb-4">
                      <span className="text-sm text-gray-600">
                        <span className="font-medium">Hãng:</span> {part.brand || part.supplierInfo?.name || 'N/A'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-semibold text-blue-600">{formatVND(part.pricing.retail)}</span>
                      </div>
                      <div className="flex items-center justify-between text-sm">
                      ...
                      {part.inventory.currentStock > 0 && (
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-gray-900">{part.inventory.currentStock} chiếc</span>
                        </div>
                      )}
                    </div>
                    ...
                    {part.warranty && (
                      <div className="bg-blue-50 border border-blue-200 rounded-md p-3 mb-4">
                        ...
                        <p className="text-xs text-blue-600">{part.warranty.description}</p>
                      </div>
                    )}
                    ...
                    {part.specifications && (
                      <div className="border-t border-gray-100 pt-3 mt-3">
                        <p className="text-xs font-medium text-gray-700 mb-2">Thông số kỹ thuật:</p>
                        <div className="space-y-1">
                          {part.specifications.voltage && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">Điện áp:</span>
                              <span className="text-gray-900">{part.specifications.voltage}V</span>
                            </div>
                          )}
                          {part.specifications.capacity && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">Dung lượng:</span>
                              <span className="text-gray-900">{part.specifications.capacity}kWh</span>
                            </div>
                          )}
                          {part.specifications.other?.power && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">Công suất:</span>
                              <span className="text-gray-900">{part.specifications.other.power}</span>
                            </div>
                          )}
