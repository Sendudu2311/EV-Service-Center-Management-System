...
const VehicleDetails: React.FC<VehicleDetailsProps> = ({
  ...
  const getServiceStatusBadge = (status: string) => {
    const statusColors = {
      ...
      pending: 'bg-dark-100 text-gray-800',
      cancelled: 'bg-red-100 text-red-800'
    };

    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs text-text-muted ${
        statusColors[status as keyof typeof statusColors] || 'bg-dark-100 text-gray-800'
      }`}>
    ...
    return (
      <div className="flex items-center">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg
            key={star}
            className={`h-4 w-4 ${star <= rating ? 'text-yellow-400' : 'text-text-secondary'}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
        ...
        <span className="ml-1 text-sm text-text-secondary">({rating}/5)</span>
      </div>
    );
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div className="fixed inset-0 bg-dark-9000 bg-opacity-75 transition-opacity" onClick={onClose} />
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-dark-300 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-4xl">
          <div className="bg-dark-300 px-4 pb-4 pt-5 sm:p-6">
            {/* Header */}
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-text-muted">VIN: {vehicle.vin}</p>
              </div>
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
              ...
              <div className="lg:col-span-2">
                <div className="bg-dark-900 rounded-lg p-6">
                  <h4 className="text-lg text-text-muted text-white mb-4">Vehicle Information</h4>
                  
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      <dt className="text-sm text-text-muted text-text-muted">Color</dt>
                      <dd className="mt-1 text-sm text-white">{vehicle.color}</dd>
                    </div>
                    <div>
                      <dt className="text-sm text-text-muted text-text-muted">Purchase Date</dt>
                      <dd className="mt-1 text-sm text-white">{formatDate(vehicle.purchaseDate)}</dd>
                    </div>
                    {vehicle.warrantyExpiry && (
                      <div>
                        <dt className="text-sm text-text-muted text-text-muted">Warranty Expiry</dt>
                        <dd className="mt-1 text-sm text-white">{formatDate(vehicle.warrantyExpiry)}</dd>
                      </div>
                    )}
                  </div>

                  <div className="mt-6">
                    <h5 className="text-md text-text-muted text-white mb-3">Technical Specifications</h5>
                    <div className="grid grid-cols-2 gap-6">
                      <div className="flex items-center">
                        ...
                        <div>
                          <dt className="text-sm text-text-muted text-text-muted">Battery</dt>
                      ...
                      <div className="flex items-center">
                        ...
                        <div>
                          <dt className="text-sm text-text-muted text-text-muted">Max Charging Power</dt>
                          <dd className="text-sm text-white">{vehicle.maxChargingPower} kW</dd>
                        </div>
                      </div>
                      <div className="flex items-center">
                        <svg className="h-5 w-5 text-lime-500 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                        </svg>
                        <div>
                          <dt className="text-sm text-text-muted text-text-muted">Range</dt>
                          <dd className="text-sm text-white">{vehicle.range} km</dd>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {/* Maintenance History */}
                <div className="mt-6 bg-dark-900 rounded-lg p-6">
                  <h4 className="text-lg text-text-muted text-white mb-4">Maintenance History</h4>
                  ...
                  {loadingHistory ? (
                    ...
                    <p className="text-sm text-text-muted">No maintenance history available</p>
                  ) : (
                    <div className="space-y-4">
                      {maintenanceHistory.map((record) => (
                        <div key={record._id} className="bg-dark-300 rounded-lg border p-4">
                ...
                {/* Vehicle Images */}
                <div className="mt-6 bg-dark-900 rounded-lg p-6">
                  <h4 className="text-lg text-text-muted text-white mb-4">Vehicle Images</h4>
                  <VehicleImageGallery
                ...
                <div className="space-y-6">
                  ...
                  <div className={`rounded-lg border-2 p-4 ${getMaintenanceStatusColor(vehicle)}`}>
                    <div className="flex items-center">
                      ...
                      <h4 className="text-sm text-text-muted">Maintenance Status</h4>
                    </div>
                  ...
                  {/* Current Mileage */}
                  <div className="bg-dark-300 rounded-lg border p-4">
                    <div className="flex items-center justify-between">
                      <h4 className="text-sm text-text-muted text-white">Current Mileage</h4>
                      <button
                    ...
                    <div className="mt-2 text-xs text-text-muted">
                      <p className="text-white">Service interval: {vehicle.maintenanceInterval.toLocaleString()} km</p>
                      <p className="text-white">Time-based: {vehicle.timeBasedInterval} months</p>
                    </div>
                    ...
                    {showMileageUpdate && (
                      <div className="mt-3 space-y-2">
                        <input
                          ...
                          className="block w-full rounded-md border-dark-300 text-sm"
                        ...
                        <div className="flex space-x-2">
                          ...
                          <button
                            ...
                            className="flex-1 bg-dark-300 text-text-secondary px-2 py-1 rounded text-xs hover:bg-dark-200"
                          >
                            Cancel
                          </button>
                        </div>
