...
import { socketAuth, validateAppointmentAccess, validateChatMessage, sanitizeMessage } from './middleware/socketAuth.js';
...
io.on('connection', (socket) => {
  console.log(`User connected: ${socket.id} (${socket.user.email}, ${socket.userRole})`);

  // Join appointment room with access validation
  socket.on('join_appointment', async (appointmentId) => {
    try {
      const { canJoin, message } = await validateAppointmentAccess(socket, appointmentId);
  ...
  socket.on('send_message', async (data) => {
    try {
      ...
      const { canJoin } = await validateAppointmentAccess(socket, data.appointmentId);
      if (!canJoin) {
        socket.emit('message_error', { message: 'Access denied to this appointment' });
        return;
      }
      ...
      const messageData = {
        ...
        senderName: socket.user.fullName || `${socket.user.firstName} ${socket.user.lastName}`,
      ...
      io.to(`appointment_${data.appointmentId}`).emit('receive_message', messageData);
  ...
  socket.on('appointment_update', async (data) => {
    try {
      ...
      if (!['staff', 'technician', 'admin'].includes(socket.userRole)) {
        socket.emit('error', { message: 'Unauthorized to update appointment status' });
        return;
      }
  ...
  socket.on('service_progress_update', async (data) => {
    try {
      ...
      if (!['technician', 'admin'].includes(socket.userRole)) {
        socket.emit('error', { message: 'Unauthorized to update service progress' });
        return;
      }
      ...
      io.to(`appointment_${data.appointmentId}`).emit('service_progress_updated', {
        appointmentId: data.appointmentId,
      ...
      console.log(`Service progress updated for appointment ${data.appointmentId} by ${socket.userId}`);
    } catch (error) {
