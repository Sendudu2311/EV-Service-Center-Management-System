...
export const checkNoShowAppointments = async () => {
  try {
    ...
    console.log(`[${now.toISOString()}] Checking for no-show appointments before ${cutoffTime.toISOString()}`);
    ...
    for (const appointment of overdueAppointments) {
      try {
        ...
        console.log(`✅ Appointment ${appointment.appointmentNumber} marked as no-show`);
        ...
        if (socketInstance) {
          ...
          socketInstance.to(`service_center_${appointment.serviceCenterId._id}`).emit('appointment_status_updated', notificationData);

          // Notify assigned technician if any
          if (appointment.assignedTechnician) {
            socketInstance.to(`user_${appointment.assignedTechnician._id}`).emit('appointment_no_show', {
              ...notificationData,
        ...
        console.error(`❌ Failed to mark appointment ${appointment.appointmentNumber} as no-show:`, error.message);
        results.push({
          appointmentId: appointment._id,
          appointmentNumber: appointment.appointmentNumber,
    ...
    if (results.length > 0) {
      console.log(`✅ No-show check completed. Processed ${results.length} appointments:`,
...
export const checkUpcomingAppointments = async () => {
  try {
    ...
    console.log(`[${now.toISOString()}] Checking for upcoming appointments at ${reminderTime.toISOString()}`);
    ...
    .populate('customerId', 'firstName lastName email phone')
    .populate('serviceCenterId', 'name contact')
    .populate('assignedTechnician', 'firstName lastName');
      ...
      try {
        ...
        if (socketInstance) {
          const reminderData = {
            ...
            scheduledDate: appointment.scheduledDate,
            scheduledTime: appointment.scheduledTime,
            minutesUntil: Math.round((appointment.scheduledDate - now) / (1000 * 60)),
            reminderCount: appointment.remindersSent,
            timestamp: now
          };

          // Send reminder to customer
          socketInstance.to(`user_${appointment.customerId._id}`).emit('appointment_reminder', {
            ...reminderData,
          ...
          socketInstance.to(`service_center_${appointment.serviceCenterId._id}`).emit('upcoming_appointment', {
            ...reminderData,
          ...
          if (appointment.assignedTechnician) {
            socketInstance.to(`user_${appointment.assignedTechnician._id}`).emit('upcoming_appointment', {
              ...reminderData,
              technicianId: appointment.assignedTechnician._id,
        ...
        console.log(`✅ Reminder sent for appointment ${appointment.appointmentNumber} (${appointment.remindersSent}/2)`);
    ...
    if (results.length > 0) {
      console.log(`✅ Appointment reminders completed. Processed ${results.length} appointments`);
    }
