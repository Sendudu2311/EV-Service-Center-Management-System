...
// Most routes require authentication, but return and IPN routes are public
// router.use(protect); // Moved to individual routes that need auth
  ...
  try {
    ...
    if (pendingPayment) {
      console.log(
        ...
        JSON.stringify(pendingPayment, null, 2)
      );
      ...
      if (success) {
        ...
        try {
          ...
          if (user) {
            ...
            const serviceIds =
              pendingPayment.appointmentData?.services?.map(
                (s) => s.serviceId
              ) || [];
              ...
              pendingPayment.appointmentData?.services?.map(
                (appointmentService) => {
                  const serviceDetails = services.find(
                    (s) =>
                      s._id.toString() ===
                      appointmentService.serviceId.toString()
                  );
                  return {
            ...
            const appointmentData = {
              appointmentNumber: `APT${Date.now()}`, // Temporary number, will be updated when appointment is created
            ...
            // Execute payment success workflow (emails will be sent)
            await executePaymentSuccessWorkflow(
              paymentData,
              appointmentData,
              user
            );
        ...
        console.log(
          "Payment successful, waiting for frontend to complete appointment booking"
        );
      }
      ...
      const redirectUrl = `${
        ...
        createdAppointment ? `&appointmentId=${createdAppointment._id}` : ""
      }`;
      return res.redirect(redirectUrl);
    } else {
  ...
  try {
    ...
    if (pendingPayment) {
      ...
      if (success) {
        ...
        // Send payment success notifications via IPN
        try {
          // Get user data for notifications
          const user = await User.findById(pendingPayment.userId);
          if (user) {
            ...
            const paymentData = {
              ...
              paymentDate: new Date(),
              vnpayTransaction: pendingPayment.vnpayTransaction,
              services: pendingPayment.appointmentData?.services || [],
            };
            ...
            console.log("✅ IPN - Payment success notifications sent");
          }
        } catch (notificationError) {
          console.error(
            "❌ IPN - Failed to send payment notifications:",
            notificationError
          );
          // Don't fail the payment process if notifications fail
        }
        ...
        // The frontend will handle appointment creation after payment verification.
        console.log(
          "IPN - Payment successful, waiting for frontend to complete appointment booking"
        );
      }
 ...
 * @access  Private
 */
router.get("/payment-methods", protect, async (req, res, next) => {
  try {
 ...
 * @access  Private
 */
router.post("/check-transaction", protect, async (req, res, next) => {
  try {
 ...
 * @access  Private
 */
router.post("/verify-appointment-payment", protect, async (req, res, next) => {
  try {
    const { transactionRef } = req.body;
 ...
 * @access  Private
 */
router.get("/debug-pending-payments", protect, async (req, res) => {
  try {
...
/**
 * @route   POST /api/vnpay/test-email-notifications
 * @desc    Test email notifications for a specific transaction
 * @access  Private
 */
router.post("/test-email-notifications", protect, async (req, res) => {
  try {
    const { transactionRef } = req.body;
    ...
    // Execute payment success workflow
    const result = await executePaymentSuccessWorkflow(
      paymentData,
      appointmentData,
      user
    );
    ...
    console.error("Test email notifications error:", error);
    res.status(500).json({
      success: false,
      message: "Test failed",
      error: error.message,
...
/**
 * @route   POST /api/vnpay/test-email-direct
 * @desc    Test email notifications directly without pending payment
 * @access  Private
 */
 ...
 * @access  Private
 */
router.post("/test-appointment-creation", protect, async (req, res) => {
  try {
    ...
    const appointmentNumber = `APT${String(appointmentCount + 1).padStart(
      6,
      "0"
    )}`;

    const scheduledDateTime = new Date(
      `${testAppointmentData.scheduledDate}T${testAppointmentData.scheduledTime}:00`
    );
