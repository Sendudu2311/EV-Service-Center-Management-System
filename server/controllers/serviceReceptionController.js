...
export const createServiceReception = async (req, res) => {
  try {
    ...
    if (existingReception) {
      return sendError(res, 400, 'Service reception already exists for this appointment', null, 'RECEPTION_EXISTS');
    }
    ...
    if (!appointment) {
      return sendError(res, 404, 'Appointment not found', null, 'APPOINTMENT_NOT_FOUND');
    }

    // Verify appointment status
    if (appointment.status !== 'customer_arrived') {
      return sendError(res, 400, 'Cannot create service reception. Customer must be marked as arrived first', null, 'INVALID_STATUS');
    }
    ...
    if (requestedServices && requestedServices.length > 0) {
      ...
      if (services.length !== serviceIds.length) {
        return sendError(res, 400, 'One or more requested services not found', null, 'INVALID_SERVICES');
      }
    }
    ...
    if (requestedParts && requestedParts.length > 0) {
      ...
      if (parts.length !== partIds.length) {
        return sendError(res, 400, 'One or more requested parts not found', null, 'INVALID_PARTS');
      }

      // Check stock availability
      for (const requestedPart of requestedParts) {
        const part = parts.find(p => p._id.toString() === requestedPart.partId);
        if (part.stockQuantity < requestedPart.quantity) {
          return sendError(res, 400, `Insufficient stock for part: ${part.name}`, null, 'INSUFFICIENT_STOCK');
        }
      }
    }
    ...
    const populatedReception = await ServiceReception.findById(serviceReception._id)
    ...
    return sendError(res, 500, 'Error creating service reception', null, 'INTERNAL_SERVER_ERROR');
  }
};
...
export const getServiceReception = async (req, res) => {
  try {
    ...
    if (!serviceReception) {
      return sendError(res, 404, 'Service reception not found', null, 'RECEPTION_NOT_FOUND');
    }

    return sendSuccess(res, 200, 'Service reception retrieved successfully', serviceReception);

  } catch (error) {
    console.error('Error retrieving service reception:', error);
    return sendError(res, 500, 'Error retrieving service reception', null, 'INTERNAL_SERVER_ERROR');
  }
};
...
export const updateServiceReception = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 200, 'Service reception updated successfully', populatedReception);

  } catch (error) {
    console.error('Error updating service reception:', error);
    return sendError(res, 500, 'Error updating service reception', null, 'INTERNAL_SERVER_ERROR');
  }
};
...
export const approveServiceReception = async (req, res) => {
  try {
    ...
    const Appointment = mongoose.model('Appointment');
    const appointment = await Appointment.findById(serviceReception.appointmentId);
    if (appointment) {
    ...
    return sendSuccess(res, 200, `Service reception ${approved ? 'approved' : 'rejected'} successfully`, populatedReception);

  } catch (error) {
    console.error('Error approving service reception:', error);
    return sendError(res, 500, 'Error approving service reception', null, 'INTERNAL_SERVER_ERROR');
  }
};