...
export const listSlots = async (req, res) => {
  try {
    ...
    return sendSuccess(res, 200, 'Slots fetched', { count: slots.length, data: slots });
  } catch (err) {
    console.error(err);
...
export const createSlot = async (req, res) => {
  try {
    ...
    if (existingExact) {
      return sendError(res, 400, 'A slot already exists for this exact time period');
    }
    ...
    if (overlapping) {
      return sendError(res, 400, `Slot overlaps with existing slot (${overlapping.startTime} - ${overlapping.endTime})`);
    }
...
export const updateSlot = async (req, res) => {
  try {
    ...
    ['technicianId', 'start', 'end', 'capacity', 'status', 'meta'].forEach(field => {
...
export const assignTechnicians = async (req, res) => {
  try {
    ...
    technicianIds = technicianIds.map(id => (typeof id === 'string' ? id : id._id || id.id)).filter(Boolean);
    ...
    if (technicianIds.length > 0) {
      ...
      if (validTechnicians.length !== technicianIds.length) {
        return sendError(res, 400, 'Some technician IDs are invalid or inactive');
      }
    }
    ...
    return sendSuccess(res, 200, 'Technician assignment updated', { data: slot });
  } catch (err) {
    console.error(err);
