    ...
    transactions: [
      {
        type: mongoose.Schema.Types.ObjectId,
        ref: "Transaction",
        description: "All transactions related to this invoice",
      },
    ],
...
invoiceSchema.methods.calculateTotals = function () {
  ...
  this.totals.subtotalAdditional = this.additionalCharges.reduce(
    (sum, charge) => {
  ...
  this.totals.remainingAmount =
    this.totals.totalAmount - (this.totals.depositAmount || 0);

  // Update payment remaining amount
  this.paymentInfo.remainingAmount =
    this.totals.remainingAmount - (this.paymentInfo.paidAmount || 0);
...
invoiceSchema.methods.recordPayment = async function (paymentData) {
  ...
  if (appointment) {
    appointment.paymentStatus =
      this.paymentInfo.status === "paid" ? "paid" : "partial";
...
// serviceCenterId index removed - single center architecture
