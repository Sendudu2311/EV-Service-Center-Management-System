...
const vnpayTransactionSchema = new mongoose.Schema({
  ...
  paymentType: {
    type: String,
    required: true,
    enum: ['appointment', 'invoice', 'service', 'other'],
  ...
  serviceCenterId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'ServiceCenter',
    description: "Service center where payment was made"
  },
...
vnpayTransactionSchema.methods.updateStatus = function(newStatus, additionalData = {}) {
...
vnpayTransactionSchema.methods.handleFailure = function(responseCode, errorMessage) {
  this.status = 'failed';
