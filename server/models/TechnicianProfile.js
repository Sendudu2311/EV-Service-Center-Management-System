...
const technicianProfileSchema = new mongoose.Schema({
  ...
  availability: {
    status: {
      type: String,
      enum: ['available', 'busy', 'break', 'offline', 'sick_leave', 'vacation'],
      default: 'available'
    },
    currentAppointment: {
      type: mongoose.Schema.Types.ObjectId,
...
technicianProfileSchema.virtual('workloadPercentage').get(function() {
  return this.workload.capacity > 0 ? (this.workload.current / this.workload.capacity) * 100 : 0;
});
...
technicianProfileSchema.virtual('averageSkillLevel').get(function() {
  ...
  const total = this.skillMatrix.reduce((sum, skill) => sum + skill.proficiencyLevel, 0);
  return total / this.skillMatrix.length;
});

// Method to update performance metrics
technicianProfileSchema.methods.updatePerformanceMetrics = async function(appointmentData) {
  const completionTime = appointmentData.actualCompletion - appointmentData.scheduledDate;
  ...
  const totalTime = this.performance.averageCompletionTime * (this.performance.completedJobs - 1);
  this.performance.averageCompletionTime = Math.floor((totalTime + completionTimeMinutes) / this.performance.completedJobs);
  
  // Update customer rating if provided
  if (appointmentData.feedback && appointmentData.feedback.rating) {
    const currentTotal = this.performance.customerRating * (this.performance.completedJobs - 1);
    this.performance.customerRating = (currentTotal + appointmentData.feedback.rating) / this.performance.completedJobs;
  }
...
technicianProfileSchema.methods.isAvailableForAppointment = function(scheduledDate, estimatedDuration) {
  ...
  // Check working hours
  const appointmentDay = scheduledDate.toLocaleDateString('en-US', { weekday: 'long' }).toLowerCase();
...
technicianProfileSchema.methods.completeAppointment = function(appointmentId) {
  ...
  if (this.availability.currentAppointment && this.availability.currentAppointment.equals(appointmentId)) {
